...
# Part of the inference model: this is q(y|x) where probability of cluster y for an input x
...
for i in xrange(k):
    with tf.name_scope('graphs/hot_at{:d}'.format(i)):
        ...
        # Part of the inference model: This is q(z|x,y), 
        # parametrized by a gaussian of with mean zm and variance zv
        z[i], zm[i], zv[i] = qz_graph(xb, y)
        # Part of generative model: This is p(x|z,y)
